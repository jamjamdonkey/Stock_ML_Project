...
# 3. 슬라이싱 함수 (10일 버전)
def create_sliding_dataset(df, window_size=10):
    samples = []
...
# 4. 생성 및 저장
train_10 = create_sliding_dataset(df, window_size=10)
train_10.to_csv("Train_10days.csv", index=False, encoding='utf-8-sig')

# 5. 확인 (선택)
print("총 샘플 수:", len(train_10))
print("종목 수:", train_10['종목코드'].nunique())
print(train_10.head())
