...
learn_path = os.path.join(BASE_DIR, "Learn_3D.csv")
input_path = os.path.join(BASE_DIR, "Input_3D.csv")
output_path = os.path.join(BASE_DIR, "Top10_Recommanded_3D.csv")
...
code_input = input_df['종목코드'].astype(str).str.zfill(6).values  # 코드 포맷 보정
...
input_df['종목코드'] = input_df['종목코드'].astype(str).str.zfill(6)  # zfill 보정
input_df['종목명'] = input_df['종목코드'].map(stock_name_dict)        # ✅ 종목명 매핑
...
print("\n예측 완료 및 SHAP 기반 상위 10종목 설명 제공됨")

# [10] SHAP 상위 피처 3개 정보도 포함해서 CSV 저장
shap_features = []
...
print("\nTop10_Recommanded_3D.csv 저장 완료 (SHAP 상위 피처 + 종목명 포함)")
